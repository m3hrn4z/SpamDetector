...
class Utility:
    ...
    def extract_file_class_from_its_name(self, file_name):
        if (file_name.find('spam') != -1):
            return 'spam'
        else:
            return 'ham'

    def preprocess_file_and_create_vocabulary(self, filename):
        print(filename)
        f = open(filename, "rb")
        text = f.read()
        f.close()
    ...
    def create_frequency_table(self, corpus_folder):
        files_list = self.list_directory_files(corpus_folder)
        vocab_freq_class_dict = {}
        for file_name in files_list:
            file_category = self.extract_file_class_from_its_name(file_name)
            list_of_vocabs_in_file = self.preprocess_file_and_create_vocabulary(file_name)
